...
function batismoChange(obj, cid)
{
	...
	if (diff >= 0)
	{
		...
		{
			oi = oname + '-' + i + '-';
			$(star).append('<input type="hidden" name="' + oname + '" value="' + i + '" /><div class="tr1"><div class="td1"><b>' + cid + '<br />Batismo #'+ (i + 1) + '</b></div><table><tr><td align="right">Nome Completo:</td><td align="left"><input size="35" name="' + oi + 'name" type="text" /></td></tr><tr><td align="right">Idade:</td><td align="left"><input name="' + oi + 'age" size="4" type="text" onchange="numeroChange(this);" /></td></tr><tr><td align="right">Data do Batismo:</td><td align="left"><select name="' + oi + 'date">{{ dopt }}</select></td></tr><tr><td align="right">Sexo:</td><td align="left"><select name="' + oi + 'sex"><option value="">---------</option><option value="Masculino">Masculino</option><option value="Feminino">Feminino</option></select></td></tr></table><div class="space-line"></div></div>').find("div.tr1:last").hide().slideDown("slow");
		}
	}
	else
	{
