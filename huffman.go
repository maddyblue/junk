...
// symbol is found.
func (t *huffmanTree) Decode(br *bitReader) (v uint32) {
	nodeIndex := uint32(0) // node 0 is the root of the tree.
...
func newHuffmanTree(lengths []uint32) (huffmanTree, error) {
...
// constructed node.
func buildHuffmanNode(t *huffmanTree, codes []huffmanCode, level uint32) (nodeIndex uint32, err error) {
	...
	if len(left) == 0 || len(right) == 0 {
		...
		if len(codes) < 2 {
			return 0, errors.New("vorbis: empty Huffman tree")
		}
		...
		if level == 31 {
			...
			// infinite recursion.
			return 0, errors.New("vorbis: equal symbols in Huffman tree")
		}
